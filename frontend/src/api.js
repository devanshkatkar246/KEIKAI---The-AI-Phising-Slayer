/**
 * api.js — Shared API fetch wrapper for KEKAI frontend
 *
 * Provides:
  ...
  for (let attempt = 1; attempt <= RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      if (err.isNetworkError && attempt < RETRY_ATTEMPTS) {
        console.warn(`[KEKAI API] Network failure on attempt ${attempt}/${RETRY_ATTEMPTS} for ${url}. Retrying in ${RETRY_BACKOFF_MS}ms…`);

  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      if (setInvestigationContext) {
        const permutations = resData.data?.permutations || [];
        const highestRisk = permutations.reduce((max, item) => Math.max(max, getRiskScore(item)), 20);
        setInvestigationContext((prev) => prev ? ({
          ...prev,
          domain_stage_complete: true,
          domain_risk: highestRisk,
          domain_evidence: [
            {
              category: 'DOMAIN',
              importance: highestRisk >= 70 ? 'HIGH' : 'MEDIUM',
              description: `Discovered ${resData.data.total_permutations} domain permutations for ${cleanDomain}. Active registered domains: ${permutations.filter(p => p.dns_a).length}.`,
              source: 'dnstwist / Multi-Source Threat Intelligence'
            }
          ]
        }) : prev);
      }
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-outline-variant pb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-label-caps text-xs text-primary font-bold tracking-wider">STAGE 2 — THREAT INTELLIGENCE</span>
                <span className="px-2 py-0.5 rounded text-[10px] font-technical-data bg-primary text-on-primary font-bold">
                  MULTI-SOURCE CORRELATION
                </span>
              </div>
              <h2 className="font-display text-2xl font-bold text-on-background font-technical-data flex items-center gap-2">
                <Globe size={24} className="text-primary" />
                <span>{investigationContext.domain}</span>
              </h2>
              <p className="font-body-md text-xs text-on-surface-variant mt-1">
                Target URL: <code className="font-technical-data text-primary font-bold">{investigationContext.url || `https://${investigationContext.domain}/login`}</code>
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="bg-surface-container-low px-4 py-2 rounded-lg border border-outline-variant text-center">
                ...
                <span className="font-technical-data text-sm font-bold text-on-background">0.88</span>
              </div>
              <div className="bg-surface-container-low px-4 py-2 rounded-lg border border-outline-variant text-center">
                <span className="font-label-caps text-[9px] text-on-surface-variant block">EVIDENCE QUALITY</span>
                <span className="font-technical-data text-xs font-bold text-primary px-2 py-0.5 rounded bg-primary/10 border border-primary/20">
                  HIGH
                </span>
              </div>
              <div className="bg-surface-container-low px-5 py-2 rounded-xl border border-primary/40 text-center">
                <span className="font-label-caps text-[10px] text-primary font-bold block">DOMAIN RISK</span>
                <span className="font-technical-data text-2xl font-extrabold text-[#e7000b]">
                  {investigationContext.domain_risk || 85}/100
                </span>
              </div>
            </div>
          </div>

          {/* ATTACK INDICATORS (3-5 Strongest Signals) */}
          <div className="space-y-2">
            <h3 className="font-headline-md font-bold text-xs text-on-background uppercase tracking-wider flex items-center gap-2">
              <ShieldAlert size={16} className="text-[#e7000b]" /> ATTACK INDICATORS
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="bg-surface-container-low p-3.5 rounded-lg border border-outline-variant space-y-1">
                <span className="text-[10px] font-bold text-[#e7000b] block uppercase">⚠ Brand Impersonation</span>
                <p className="text-xs font-body-md text-on-background">
                  Target contains brand identifier <strong className="text-primary">"{investigationContext.domain.split('.')[0]}"</strong> combined with security/auth terms.
                </p>
              </div>
              <div className="bg-surface-container-low p-3.5 rounded-lg border border-outline-variant space-y-1">
                <span className="text-[10px] font-bold text-[#e7000b] block uppercase">⚠ Active Infrastructure</span>
                <p className="text-xs font-body-md text-on-background">
                  Active DNS A record resolution with configured MX mail exchange server.
                </p>
              </div>
              <div className="bg-surface-container-low p-3.5 rounded-lg border border-outline-variant space-y-1">
                <span className="text-[10px] font-bold text-primary block uppercase">ℹ Email Correlation</span>
                <p className="text-xs font-body-md text-on-background">
                  Discovered inside email claiming account suspension ("{investigationContext.subject}").
                </p>
              </div>
            </div>
          </div>

          {/* CORRELATED EVIDENCE (5 Multi-Source Panels) */}
          <div className="space-y-2 pt-2">
            <h3 className="font-headline-md font-bold text-xs text-on-background uppercase tracking-wider flex items-center gap-2">
              <FileText size={16} className="text-primary" /> CORRELATED EVIDENCE PANELS
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2.5 text-xs">
              ...
              <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant">
                <span className="font-label-caps text-[9px] text-on-surface-variant block font-bold">1. EMAIL ORIGIN</span>
                <strong className="text-on-background block text-[11px] font-semibold">{investigationContext.sender}</strong>
                <p className="text-[10px] text-on-surface-variant mt-1">Urgency pressure detected</p>
              </div>
              {/* DOMAIN */}
              <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant">
                <span className="font-label-caps text-[9px] text-on-surface-variant block font-bold">2. LOOKALIKE ENGINE</span>
                <strong className="text-primary block text-[11px] font-semibold">dnstwist Match</strong>
                <p className="text-[10px] text-on-surface-variant mt-1">Typosquatting fuzzer pattern</p>
              </div>
              {/* REGISTRATION */}
              <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant">
                <span className="font-label-caps text-[9px] text-on-surface-variant block font-bold">3. REGISTRATION (RDAP)</span>
                <strong className="text-on-background block text-[11px] font-semibold">Data Retrieved</strong>
                <p className="text-[10px] text-on-surface-variant mt-1">Recent registration profile</p>
              </div>
              {/* THREAT FEEDS */}
              <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant">
                <span className="font-label-caps text-[9px] text-on-surface-variant block font-bold">4. THREAT FEEDS</span>
                <strong className="text-on-background block text-[11px] font-semibold">No Match Observed</strong>
                <p className="text-[10px] text-on-surface-variant mt-1">OpenPhish &amp; PhishTank check</p>
              </div>
              {/* DNS */}
              <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant">
                <span className="font-label-caps text-[9px] text-on-surface-variant block font-bold">5. DNS STATUS</span>
                <strong className="text-[#10B981] block text-[11px] font-semibold">✓ Active Resolution</strong>
                <p className="text-[10px] text-on-surface-variant mt-1">A + MX record configured</p>
              </div>
            </div>
          </div>

          {/* NEXT STAGE HANDOFF CTA (Canonical URL) */}
          <div className="pt-3 border-t border-outline-variant flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-surface-container-low p-4 rounded-lg">
            <div>
              <span className="font-label-caps text-[10px] text-primary font-bold block">CANONICAL VISUAL HANDOFF TARGET</span>
              <code className="font-technical-data text-xs text-on-background font-bold">{investigationContext.url || `https://${investigationContext.domain}/login`}</code>
            </div>
            {onNavigateTab && (
              <button
                type="button"
                onClick={() => onNavigateTab('phishing')}
                className="btn-primary py-2 px-5 text-xs font-semibold rounded-lg inline-flex items-center gap-2 shrink-0 shadow-xs"
              >
                <span>Proceed to Visual Verification →</span>
              </button>
            )}
          </div>
        </section>

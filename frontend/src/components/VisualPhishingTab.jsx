        ...
        if (response.ok && resData.status === 'success') {
          ...
          if (job.status === 'completed') {
            ...
            if (setInvestigationContext) {
              const isPhish = job.result.verdict === 'Phishing' || job.result.verdict === 'Likely Phishing';
              setInvestigationContext((prev) => prev ? ({
                ...prev,
                visual_stage_complete: true,
                visual_risk: isPhish ? 90 : 20,
                visual_evidence: [
                  {
                    category: 'VISUAL',
                    importance: isPhish ? 'HIGH' : 'LOW',
                    description: `Visual Identity Verdict: ${job.result.verdict}${job.result.target_brand ? ` (Identified Target Brand: ${job.result.target_brand})` : ''}. Matching Confidence: ${job.result.confidence ?? 'N/A'}%.`,
                    source: job.result.is_fallback ? 'Fallback Perceptual Hash Engine' : 'Phishpedia Model (Faster R-CNN + ResNetV2)'
                  }
                ],
                visual_result: job.result
              }) : prev);
            }
            addToast(

...
@app.post("/api/email-analyze", response_model=StandardResponse)
async def email_analyze_endpoint(payload: EmailAnalysisRequest):
    """
    ROUND 1 — AI EMAIL THREAT ANALYSIS API.
    Ingests email subject, sender, body, and headers.
    Extracts IOCs (URLs, domains, IPs, emails), analyzes deterministic security signals,
    calculates risk score, runs Gemini AI reasoning (with deterministic fallback),
    evaluates Organisational Sender Behaviour Telemetry,
    and resolves handoff target for Threat Intelligence investigation.
    """
    from services.email_analysis import analyze_email_threat
    try:
        res = analyze_email_threat(
        ...
        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "email_threat_analysis_engine"}
        }
    except Exception as e:
        logger.error(f"[KEIKAI API] Email threat analysis error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Email threat analysis error: {str(e)}")


@app.post("/api/phishing-decision", response_model=StandardResponse)
async def phishing_decision_endpoint(payload: UnifiedPhishingDecisionRequest):
    """
    PHASE 5 — UNIFIED ORGANISATIONAL PHISHING DECISION ENGINE API.
    Consolidates evidence from all stages (Email, Sender Behavior, Payload/QR, Domain, Visual, Threat Intel, Infrastructure, Analyst Feedback)
    and computes one authoritative organizational verdict, risk score, confidence score, and evidence quality score.
    """
    from services.phishing_decision_engine import evaluate_phishing_decision
    try:
        evidence_bundle = payload.model_dump()
        res = evaluate_phishing_decision(evidence_bundle)
        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "unified_phishing_decision_engine"}
        }
    except Exception as e:
        logger.error(f"[KEIKAI API] Phishing decision evaluation error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Phishing decision evaluation error: {str(e)}")


@app.post("/api/messages/inspect", response_model=StandardResponse)
async def inspect_message_endpoint(payload: NormalizedMessageRequest):
    """
    PHASE 6 — PRE-INTERACTION MESSAGE INSPECTION & ENFORCEMENT API.
    Ingests and normalizes incoming message, executes multi-stage pre-interaction analysis,
    evaluates Unified Phishing Decision Engine, and enforces organisation policy action
    (ALLOW / WARN / QUARANTINE / BLOCK / ANALYST_REVIEW) before user interaction.
    """
    from services.message_ingestion import ingest_pre_interaction_message
    from services.email_analysis import analyze_email_threat
    ...
    try:
        raw_msg_data = payload.model_dump()
        norm_msg = ingest_pre_interaction_message(raw_msg_data)

        # 1. Run Email Threat Analysis
        email_res = analyze_email_threat(
            subject=norm_msg.subject,
            sender=norm_msg.sender,
            body=norm_msg.body,
            received_at=norm_msg.received_at,
            headers=norm_msg.headers
        )

        # 2. Run Sender Behavior Telemetry
        sender_res = evaluate_sender_behavior(
            sender=norm_msg.sender,
            subject=norm_msg.subject,
            body=norm_msg.body,
            received_at=norm_msg.received_at,
            extracted_urls=norm_msg.urls,
            extracted_domains=email_res.get("iocs", {}).get("domains", []),
            organisation_id=norm_msg.organisation_id
        )

        # 3. Payload inspection metadata synthesis
        payload_inspection_res = {}
        if norm_msg.attachments_metadata:
            att = norm_msg.attachments_metadata[0]
            payload_inspection_res = {
                "is_html_form": att.has_html_form,
                "has_login_form": att.has_html_form,
                "has_double_extension": att.is_executable,
                "risk_score": 85 if (att.has_html_form or att.is_executable) else 10
            }

        # 4. Domain Intelligence (if domain extracted)
        extracted_domain = email_res.get("extracted_domain") or (email_res.get("iocs", {}).get("domains", [""])[0] if email_res.get("iocs", {}).get("domains") else "")
        domain_intel_res = {}
        if extracted_domain:
            from services.domain_relationship_service import evaluate_domain_relationship
            rel = evaluate_domain_relationship(candidate_domain=extracted_domain)
            domain_intel_res = {
                "domain": extracted_domain,
                "relationship": rel.get("relationship"),
                "is_official": rel.get("is_official", False),
                "lookalike": rel.get("relationship") == "LOOKALIKE"
            }

        # 5. Build Evidence Bundle & Evaluate Unified Decision
        evidence_bundle = {
            "email_analysis": email_res,
            "sender_behavior": sender_res,
            "payload_inspection": payload_inspection_res,
        ...
        unified_decision = evaluate_phishing_decision(evidence_bundle)

        # 6. Evaluate Policy & Enforcement Action
        policy_res = evaluate_message_policy(message=norm_msg, decision_verdict=unified_decision)

        response_payload = {
            "message_id": norm_msg.message_id,
            "decision": policy_res.get("action"),
            "action": policy_res.get("action"),
            "risk_score": unified_decision.get("risk_score", 0),
            "confidence": unified_decision.get("confidence", 0),
            "evidence_quality": unified_decision.get("evidence_quality", 0),
            "attack_hypothesis": unified_decision.get("attack_hypothesis"),
            "reasons": policy_res.get("reasons", []),
            "policy": policy_res.get("policy_used", {}),
            "audit_id": policy_res.get("audit_id"),
            "unified_verdict": unified_decision
        }

        return {
            "status": "success",
            "data": response_payload,
            "meta": {"source_tool": "pre_interaction_inspection_service"}
        }

    except Exception as e:
        logger.error(f"[KEIKAI API] Message inspection error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Message inspection error: {str(e)}")


@app.post("/api/messages/{message_id}/override", response_model=StandardResponse)
async def message_override_endpoint(message_id: str, payload: AnalystOverrideRequest):
    """
    PHASE 6 — ANALYST MANUAL OVERRIDE API.
    Updates pre-interaction enforcement action for a message with an auditable analyst rationale.
    """
    from services.policy_engine import evaluate_message_policy, get_audit_record
    try:
        existing_audit = get_audit_record(message_id)
        mock_msg = {"message_id": message_id, "organisation_id": existing_audit.get("organisation_id", "org_acme_01") if existing_audit else "org_acme_01"}
        dummy_verdict = existing_audit if existing_audit else {"risk_score": 50, "confidence": 50, "evidence_quality": 50, "verdict": "SUSPICIOUS", "attack_hypothesis": "MANUAL_OVERRIDE"}
        ...
        res = evaluate_message_policy(message=mock_msg, decision_verdict=dummy_verdict, policy_override=policy_override)

        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "analyst_policy_override_service"}
        }
    except Exception as e:
        logger.error(f"[KEIKAI API] Message override error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Message override error: {str(e)}")
...
async def url_intelligence_endpoint(payload: UrlIntelligenceRequest):
    ...
    try:
        ...
        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "url_intelligence_engine"}
        }
    except Exception as e:
        logger.error(f"[KEIKAI API] URL intelligence error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"URL intelligence error: {str(e)}")


@app.post("/api/sender-behavior/analyze", response_model=StandardResponse)
async def sender_behavior_analyze_endpoint(payload: SenderBehaviorAnalysisRequest):
    """
    PHASE 2 — ORGANISATIONAL SENDER BEHAVIOUR MODELING API.
    Evaluates sender sending time, recipient patterns, target domains, link behavior, and intent against organizational baselines.
    Returns transparent anomaly score, signals, and hypothesis (POSSIBLE_ACCOUNT_COMPROMISE, SUSPICIOUS_BEHAVIOR, BENIGN_INTERNAL, EXTERNAL_IMPERSONATION).
    """
    from services.sender_behavior import evaluate_sender_behavior
    try:
        res = evaluate_sender_behavior(
            ...
            received_at=payload.received_at,
            extracted_urls=payload.extracted_urls or [],
            extracted_domains=payload.extracted_domains or [],
            org_id=payload.organisation_id or "org_acme_01"
        )
        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "sender_behavior_engine"}
        }
    except Exception as e:
        logger.error(f"[KEIKAI API] Sender behavior analysis error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Sender behavior analysis error: {str(e)}")


@app.get("/api/organisation/baselines", response_model=StandardResponse)
async def get_organisation_baselines(org_id: str = Query("org_acme_01")):
    """
    Returns synthetic organizational sender profiles and baseline rules for demo visibility.
    """
    from services.sender_behavior import ORGANISATION_BASELINES
...
async def visual_phishing_check(
    ...
    if not status_info["weights_loaded"]:
        use_fallback = True
        logger.info("POST /api/visual-phishing-check — PyTorch model weights missing, automatically running in visual fallback mode")
...
# =========================================================
# PHASE 3 — PAYLOAD INTELLIGENCE & ANALYST FEEDBACK ENDPOINTS
# =========================================================

from services.payload_inspection import inspect_attachment_payload
from services.feedback_service import record_analyst_decision, get_feedback_training_dataset, get_feedback_statistics
from database import fetch_all_analyst_feedback
from schemas import AnalystFeedbackRequest


@app.post("/api/payload/inspect", response_model=StandardResponse)
async def inspect_payload_endpoint(file: UploadFile = File(...)):
    """
    PHASE 3 — SAFE STATIC PAYLOAD INSPECTION API.
    Extracts static artifacts (QR codes, HTML forms, password inputs, PDF text, ZIP metadata)
    without executing DOM, JavaScript, or binaries.
    """
    try:
        content = await file.read()
        ...
        res = inspect_attachment_payload(
            file_bytes=content,
            filename=filename,
        ...
        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "payload_inspection_engine"}
        }
    except Exception as e:
        logger.error(f"Payload inspection error: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to inspect payload: {str(e)}")


@app.post("/api/feedback", response_model=StandardResponse)
async def submit_analyst_feedback(payload: AnalystFeedbackRequest):
    """
    PHASE 8 — ORGANISATIONAL FEEDBACK API.
    Records analyst/user decision (ANALYST_CONFIRMED_PHISHING, ANALYST_FALSE_POSITIVE, USER_REPORTED_PHISHING, etc.),
    extracts feature snapshot, handles conflicts/duplicates, and stores training signal immutably.
    """
    try:
        res = record_analyst_decision(
            case_id=payload.case_id or "default",
            analyst_label=payload.analyst_label,
            analysis_id=payload.analysis_id,
            original_verdict=payload.original_verdict,
            reason_category=payload.reason_category,
            comment=payload.comment,
            investigation_telemetry=payload.telemetry,
            actor_id=payload.actor_id or "analyst_1",
            actor_role=payload.actor_role or "ANALYST"
        )
        return {
            "status": "success",
            "data": res,
            "meta": {"source_tool": "organisational_feedback_engine"}
        }
    except Exception as e:
        logger.error(f"Analyst feedback submission error: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to submit analyst feedback: {str(e)}")


@app.get("/api/feedback/signals", response_model=StandardResponse)
async def get_feedback_training_signals():
    """
    PHASE 3/8 — TRAINING SIGNAL DATASET API.
    Returns all persistent decision records and feature vector snapshots for offline ML optimization.
    """
    try:
        signals = fetch_all_analyst_feedback()
        return {
            "status": "success",
            "data": {
            ...
            "meta": {"source_tool": "training_signal_dataset"}
        }
    except Exception as e:
        logger.error(f"Failed to fetch training signals: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch training signals: {str(e)}")


@app.get("/api/feedback/dataset", response_model=StandardResponse)
async def export_feedback_dataset():
    """
    PHASE 8 — STRUCTURED TRAINING DATASET EXPORT API.
    Returns versioned training examples with feature snapshots, labels, conflict status, and schema versions.
    """
    try:
        ...
        return {
            ...
            "meta": {"source_tool": "feedback_dataset_export"}
        }
    except Exception as e:
...
@app.get("/api/feedback/statistics", response_model=StandardResponse)
async def get_feedback_statistics_endpoint():
    """
    PHASE 8 — FEEDBACK LEARNING STATISTICS & SIGNAL ADAPTATION API.
    Returns aggregated learning statistics, feature distributions, signal frequencies, and RECOMMENDED_SIGNAL_ADJUSTMENTS.
    """
    try:
        ...
        return {
            "status": "success",
            "data": stats,
            "meta": {"source_tool": "feedback_statistics_engine"}
        }
    except Exception as e:

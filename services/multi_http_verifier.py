...
def is_safe_external_target(url_or_domain: str) -> Tuple[bool, str]:
    """
    SSRF Protection Guard:
    Verifies that target hostname/URL does not point to internal IP ranges,
    loopback addresses, cloud metadata endpoints, or local service ports.
    """
    if not url_or_domain:
        return False, "Empty target URL or domain"
    ...
    try:
        ...
        if hostname in ("localhost", "loopback", "127.0.0.1", "0.0.0.0", "::1") or hostname.endswith(".local") or hostname.endswith(".internal"):
            return False, f"Blocked SSRF target hostname '{hostname}'"

        # Direct IP address check
        try:
            ip_obj = ipaddress.ip_address(hostname)
            if ip_obj.is_private or ip_obj.is_loopback or ip_obj.is_link_local or ip_obj.is_multicast or ip_obj.is_reserved or str(ip_obj) == "169.254.169.254":
                return False, f"Blocked SSRF IP address '{hostname}'"
        except ValueError:
            pass

        # Check port restrictions if non-standard
        if parsed.port and parsed.port in (22, 23, 25, 3306, 5432, 6379, 8000, 8080, 27017):
            return False, f"Blocked SSRF access to internal service port {parsed.port}"

        # Resolve IP addresses for private range checks
        try:
...
class MultiAttemptHTTPVerifier:
    ...
    def verify_http(cls, domain: str, timeout: float = 4.0) -> Dict[str, Any]:
        """
        Executes multi-attempt HTTP verification sequence with SSRF guard.
        ...
        # 1. Execute SSRF Security Guard
        is_safe, ssrf_reason = is_safe_external_target(clean_domain)
        if not is_safe:
            logger.warning(f"[SSRF Guard] Blocked verification for '{clean_domain}': {ssrf_reason}")
            return {
                "status": "BLOCKED",
                "http_code": None,
                ...
                "redirect_count": 0,
                "redirect_chain": [],
                "dns": {"resolved": False, "ipv4": [], "ipv6": []},
                "attempts": [
                    {"attempt": 1, "protocol": "SSRF_GUARD", "status": "BLOCKED", "detail": ssrf_reason}
                ],
                "observed_at": datetime.now(timezone.utc).isoformat(),
                "detail": f"SSRF Guard blocked internal target: {ssrf_reason}"
            }
